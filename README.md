...
- **Sun exposure**: pick a date and drag the time slider, for the map center. Ground in shadow is darkened with a navy veil and sunlit ground is left clear, so the map and the elevation colors stay readable where the sun is. Slopes the sun only grazes fade in gradually. During golden hour (sun between 4° below and 6° above the horizon) sunlit ground glows gold. The time slider's track is colored by light phase (night, blue hour, golden hour, day) and the golden-hour times are listed under it. The slider runs from just before morning golden hour to just after evening golden hour. Shadows cast by ridges are included. The play button steps through the day. Times are in the map location's time zone.
